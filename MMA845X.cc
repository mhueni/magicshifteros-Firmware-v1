...
#ifdef HAS_MAGNETOMETER
int magnetValue[3];		// Stores the 16-bit signed value
#endif
			...
			switch (accelWaitForType)
			{
				case WAITFOR_ACCEL:
					...
					for (uint8_t i = 0; i < 3; i++) {
						accelCount[i] = ((interruptBuffer[2*i] << 8) | interruptBuffer[2*i + 1]) >> 4;	// Turn the MSB and LSB into a 12-bit value
						if (interruptBuffer[2*i] > 0x7F) {	// If the number is negative, we have to make it so manually (no 12-bit data type)
							accelCount[i] = ~accelCount[i] + 1;
							accelCount[i] *= -1;	// Transform into negative 2's complement #
						}
					}
#ifdef HAS_MAGNETOMETER
					// For loop to calculate 16-bit magnetometer values
					for (uint8_t i = 0; i < 3; i++) {
						magnetValue[i] = (interruptBuffer[2*i + 6] << 8) | interruptBuffer[2*i + 7];
					}
#endif
					break;
	...
	else
	{
		...
		for (uint8_t interruptNr = 1; interruptNr <= 2; interruptNr++)
		{
			if (digitalRead(interruptNr == 1 ? int1Pin : int2Pin))
			{
				...
				{
#ifdef HAS_MAGNETOMETER
					// Read 12 bytes from OUT_X_MSB->
					SetupInterruptI2CReadout(MMA8452_ADDRESS, 0x01, 12);
#else
					// Read 6 bytes from OUT_X_MSB->
					SetupInterruptI2CReadout(MMA8452_ADDRESS, 0x01, 6);
#endif
				...
				else if (accelActiveConfig.transientInterruptNr == interruptNr)
				{
					...
					{
						// Read 5 bytes from TRANSIENT_SRC -> PULSE_SRC
						// Only need byte 0 and 4
						SetupInterruptI2CReadout(MMA8452_ADDRESS, 0x1E, 5);
						accelWaitForType = WAITFOR_TAPANDTRANSIENT;
					}				
					else
					{	
						// Read 1 byte from TRANSIENT_SRC
...
void AccelSetup(AccelConfig *newConfig) 
{
...
#ifdef HAS_MAGNETOMETER
	if (accelActiveConfig.autoCalibrateMagnetometer) {
	...
	writeRegister(0x5B,
								(accelActiveConfig.autoCalibrateMagnetometer ? 0x80 : 0x00) |
								0x1F);
