...
void CounterMode_draw(double percent) {
	float threshold = LEDS * percent;
 	float intensity = 0.0;
	int led = 0;
	for (int i = 0; i < LEDS; i++) {
		...
		if (i < 2)
			setRGB(led, intensity * 255, 0, 0);
		else if (i < 5)
			setRGB(led, intensity * 255, intensity * 165, 0);
		else
			setRGB(led, 0, intensity * 255, 0);
	}
	updateLedsWithBlank();
}
void CounterMode_alert(float intensity) {
	setAll(intensity * 255, intensity * 255, intensity * 255);
	updateLedsWithBlank();
}
long CounterMode_setTimeout(long countdown_ms) {
	int initseq = 2 * LEDS;
	for (int i = 0; i < initseq; i++) {
		CounterMode_draw((double)i / initseq);
		delay(10);
	}
	return millis() + countdown_ms;
}
void CounterMode()
{
	...
	while (1) {
		...
		if (centerBtnClickedTime != 0) {
			timeout = CounterMode_setTimeout(countdown);
			centerBtnClickedTime = 0;
		}

		if (millis() < timeout) {
			CounterMode_draw((double)(timeout - millis()) / countdown);
		} else if (millis() < timeout + 10000) {
			CounterMode_alert(blink = !blink ? 1 : 0);
		} else {
			CounterMode_alert(0);
		}
