...
// Set this if the board has a FXOS8700 instead of a MMA8452
...
extern int magnetValue[3];  // Stores the 16-bit signed value
...
struct AccelConfig
{
	...
	bool autoCalibrateMagnetometer;

  AccelConfig() :
    accelInterruptNr(0), tapInterruptNr(0), transientInterruptNr(0),
    accelScale(0), accelDataRate(0), autoCalibrateMagnetometer(false) {}
};
...
int loadMode();
int ModeSelector(int modes, int startIdx, const char*text[]);

static bool waitButton();
void displayText(const char *text, bool wait=true, MSBitmap *font = NULL, int posX=0);
...
#include "CustomModes/Compass.cc"
#include "bmpfont.cc"
#include "colors.cc"
